...
class MonacoEditorComponent extends HTMLElement {
  ...
  get monacoUri() {
    return this._monacoUri;
  }
  
  set monacoUri(val) {
    this._monacoUri = val;
    // 如果組件已經連接到 DOM，且之前沒有初始化成功，嘗試重新初始化
    if (val && this.isConnected && !this._isReady) {
  ...
  // 渲染編輯器容器
  _render() {
    this.innerHTML = '<div class="monaco-editor-container" style="width: 100%; height: 100%;"></div>';
  }
  ...
  _initializeMonaco() {
    ...
    script.onload = () => {
      require.config({ paths: { vs: this._monacoUri } });
      this._loadMonaco();
    };
    script.onerror = () => {
  ...
  // 處理儲存快捷鍵
  _handleSaveCommand() {
    this.dispatchEvent(new CustomEvent('save-triggered', {
      detail: { 
